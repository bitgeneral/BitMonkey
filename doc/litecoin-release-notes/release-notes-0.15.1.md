Bitmonkey Core version *0.15.1* is now available from:

  <https://download.bitmonkey.org/bitmonkey-0.15.1/>
...
A number of changes to the way Bitmonkey Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 bananas per kB (0.00001 BTM). This is to prepare for dropping the minimum transaction fee to 0.1 bananas per kB (0.0001 BTM) in the next release.
