...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/bitmonkey-project/gitian.sigs.ltc.git
    git clone https://github.com/bitmonkey-project/bitmonkey-detached-sigs.git
    git clone https://github.com/devrandom/gitian-builder.git
    git clone https://github.com/bitmonkey-project/bitmonkey.git

### Bitmonkey maintainers/release engineers, suggestion for writing release notes
...
### Build and sign Bitmonkey Core for Linux, Windows, and macOS:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit bitmonkey=v${VERSION} ../bitmonkey/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-linux --destination ../gitian.sigs.ltc/ ../bitmonkey/contrib/gitian-descriptors/gitian-linux.yml
    mv build/out/bitmonkey-*.tar.gz build/out/src/bitmonkey-*.tar.gz ../

    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit bitmonkey=v${VERSION} ../bitmonkey/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-win-unsigned --destination ../gitian.sigs.ltc/ ../bitmonkey/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit bitmonkey=v${VERSION} ../bitmonkey/contrib/gitian-descriptors/gitian-osx.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-unsigned --destination ../gitian.sigs.ltc/ ../bitmonkey/contrib/gitian-descriptors/gitian-osx.yml
...
Verify the signatures
    ...
    pushd ./gitian-builder
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-linux ../bitmonkey/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-win-unsigned ../bitmonkey/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-osx-unsigned ../bitmonkey/contrib/gitian-descriptors/gitian-osx.yml
    popd
...
Codesigner only: Sign the macOS binary:
    ...
    $   xcrun altool --notarize-app --primary-bundle-id "org.bitmonkey.Bitmonkey-Qt" -u "<apple-id-email>" -p "@keychain:<apple-id-notarisation-app-specific-password>" --asc-provider <team-id-shortcode> -t osx -f bitmonkey-${VERSION}-osx.dmg
    ...
    $   xcrun stapler staple dist/Bitmonkey-Qt.app
...
Codesigner only: Commit the detached codesign payloads:
    ...
    cp dist/Bitmonkey-Qt.app/Contents/CodeResources osx/dist/Bitmonkey-Qt.app/Contents/
    git add -A
...
- Detached signatures will then be committed to the [bitmonkey-detached-sigs](https://github.com/bitmonkey-project/bitmonkey-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.

Create (and optionally verify) the signed macOS binary:

    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../bitmonkey/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-signed --destination ../gitian.sigs.ltc/ ../bitmonkey/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-osx-signed ../bitmonkey/contrib/gitian-descriptors/gitian-osx-signer.yml
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the bitmonkey.org server.
...
- Announce the release:
  ...
  - bitmonkey-dev mailing list

  - blog.bitmonkey.org blog post

  - Update title of #bitmonkey and #bitmonkey-dev on Freenode IRC

  - Optionally twitter, reddit /r/Bitmonkey, ... but this will usually sort out itself

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/bitmonkey-project/bitmonkey/releases/new) with a link to the archived release notes.
