...
The Bitmonkey Core daemon can be configured to act as a trusted "border
router", implementing the bitmonkey wire protocol and relay, making
...
The ZeroMQ feature in Bitmonkey Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitmonkeyd:
...
For instance:
    ...
    $ bitmonkeyd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bitmonkeyd.tx.raw \
...
These options can also be provided in bitmonkey.conf.
...
From the perspective of bitmonkeyd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitmonkeyd directly. Furthermore, no information is
...
using. Bitmonkeyd appends an up-counting sequence number to each
