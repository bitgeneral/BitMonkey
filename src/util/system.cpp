...
const char * const BITCOIN_CONF_FILENAME = "bitmonkey.conf";
...
#else
    const char* pszModule = "bitmonkey";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitmonkey";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bitmonkey";
#else
    // Unix-like
    return pathRet / ".bitmonkey";
#endif
#endif
}
