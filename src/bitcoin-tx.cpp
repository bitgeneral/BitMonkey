...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitmonkey-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitmonkey-tx [options] <hex-tx> [commands]  Update hex-encoded bitmonkey transaction\n" +
            "or:     bitmonkey-tx [options] -create [commands]   Create hex-encoded bitmonkey transaction\n" +
            "\n";
