    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional BTM outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes BTM and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::BTM_TO_BTM) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a BTM-to-BTM transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::BTM_ONLY;
        ...
        // If BTM-to-BTM fails, create a peg-out transaction
